...
[📄 English Paper](paper/main.tex) · [📄 中文论文](paper/main_zh.tex) · [📚 文档](docs/) · [🚀 快速开始](#-快速开始)

</div>

> ⚠️ **开发者注意**：每次完成一个独立任务（功能、修复或文档更新）后，请立即执行 `git add`、`git commit -m "..."` 并 `git push`（提交信息请参照 `docs/07_开发规范.md` 使用中文且语义清晰）。
...
>
> **注**：历史版本、旧脚本和过时报告已归档至项目根目录的 `Archive/C1_Legacy/` 中。
...
- **强制执行**：开发过程中每完成一个独立任务（功能、修复或文档更新），请立即执行 `git add`、`git commit -m "..."` 并 `git push` 同步远程，避免把多个不相关改动合并为一个提交。
- **提交信息规范**：提交信息请参照 `docs/07_开发规范.md`，使用中文并清晰描述变更目的（如“修复 X”、“添加 Y 功能”）。
- **目的**：保持分支清晰、便于代码审查与回滚，符合项目协作要求。
