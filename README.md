...
│   └── 额外补充数据集/                 # 扩展数据（外部验证用）
│
├── 🧠 src/dwts_model/                 # 核心模型代码
│   ├── config.py                      # 全局配置参数
│   ├── paper_palette.py               # 论文统一配色系统
│   ├── etl/                           # 数据工程模块
│   │   ├── data_loader.py             # 数据加载与预处理
│   │   ├── active_set.py              # 活跃选手集管理
│   │   └── fsm.py                     # 有限状态机（状态追踪）
│   ├── engines/                       # 反演引擎
│   │   ├── engine_interface.py        # 引擎统一接口
│   │   ├── rule_engine.py             # 规则引擎（百分制/排名制）
│   │   ├── lp_percent.py              # LP 反演（百分制：S3-S27）
│   │   └── milp_rank.py               # MILP 反演（排名制：S1-S2, S28+）
│   ├── sampling/                      # 贝叶斯采样模块
│   │   └── ...                        # Hit-and-Run + 重要性重采样
│   └── analysis/                      # 分析模块
│       ├── counterfactual.py          # 反事实评估
│       ├── mechanism_design.py        # 机制设计 / 帕累托优化
│       ├── daws.py                    # DAWS 动态权重系统
│       ├── ml_xai.py                  # XGBoost + SHAP 特征分析
│       └── metrics.py                 # 评估指标（Kendall's Tau 等）
│
├── 🚀 scripts/                        # 执行脚本
...
│   ├── fig_q3_*.pdf                   # Q3 相关图表
│   ├── fig_q4_*.pdf                   # Q4 相关图表
│   ├── fig_shap_*.pdf                 # SHAP 分析图表
│   └── fig_dwts_*.pdf                 # 流程图 / 架构图
│
├── 📁 outputs/                        # 分析结果输出
│   ├── *.csv                          # 数据表格
│   ├── *.tex                          # LaTeX 表格
...
# 快速测试（减少采样）
python scripts/run_full_pipeline.py --samples 500 --skip-compile

# 仅生成图表（跳过 ML 和论文编译）
python scripts/run_full_pipeline.py --skip-ml --skip-compile
```
...
| 文档 | 说明 |
|-----|------|
| [开发规范与协作指南](docs/CONTRIBUTING.md) | **Git 工作流、分支策略、提交规范** |
| [运行与复现指南](docs/运行与复现指南.md) | 详细的复现步骤与环境配置 |
| [模型一：算法设计](docs/模型一_算法设计.md) | LP/MILP 反演算法详解 |
| [模型一：综合总结](docs/模型一_综合总结.md) | 模型设计思路与结果总结 |
| [优化分析](docs/优化分析.md) | 性能优化与参数调优策略 |
| [外部验证](docs/外部验证.md) | 使用外部数据的验证分析 |
| [图表规范](docs/图表规范.md) | 论文图表视觉系统设计规范 |
| [Handoff Summary](docs/handoff_summary.md) | 项目交接记录与状态追踪 |
...
减少采样数量或跳过 ML 分析：
```bash
python scripts/run_full_pipeline.py --samples 500 --skip-ml
```
</details>
...
增大采样数或降低 σ 值（更强平滑）：
```bash
python scripts/run_full_pipeline.py --samples 4000 --smooth-sigma 0.10
```
</details>
